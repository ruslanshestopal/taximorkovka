...
-(id)init {
	if (instance != nil) {
		[NSException raise:@"View Singleton already constructed! Use getInstance instead."
                    format:@"View Singleton already constructed! Use getInstance instead."];
...
-(id<IMediator>)removeMediator:(NSString *)mediatorName {
	id<IMediator> mediator = [[[mediatorMap objectForKey:mediatorName] retain] autorelease];
	if (mediator != nil) {
		NSArray *interests = [mediator listNotificationInterests];
		for (NSString *notificationName in interests) {
