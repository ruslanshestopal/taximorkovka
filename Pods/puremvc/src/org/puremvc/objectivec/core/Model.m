...
-(id)init {
	if (instance != nil) {
		[NSException raise:@"Model Singleton already constructed! Use getInstance instead." format:@"Model Singleton already constructed! Use getInstance instead."];
...
-(id<IProxy>)removeProxy:(NSString *)proxyName {
	id<IProxy> proxy = [[[proxyMap objectForKey:proxyName] retain] autorelease];
	if (proxy != nil) {
		[proxy onRemove];
